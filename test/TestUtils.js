...
async function initializeTokenProxy(tokenProxy) {
    return tokenProxy.initialize(name, symbol, currency, decimals, {from : deployer});
}

async function initializeWalletProxy(walletProxy) {
    return walletProxy.initialize([admin_1, admin_2, admin_3], 2, 100000000, {from : deployer});
}
