...
const ERC20RupiahToken = artifacts.require("./token/ERC20RupiahTokenL");

module.exports = async function (deployer, network, accounts) {
  let token;
  switch (network) {
    case 'development':
      token = await ERC20RupiahToken.at(DeployedAddresses.dev.tokenProxyL);
      break;
    case 'ropsten':
      token = await ERC20RupiahToken.at(DeployedAddresses.ropsten.tokenProxyL);
      break;
    case 'rinkeby':
      token = await ERC20RupiahToken.at(DeployedAddresses.rinkeby.tokenProxyL);
      break;
    case 'mainnet':
      token = await ERC20RupiahToken.at(DeployedAddresses.mainnet.tokenProxyL);
      break;
  }

  console.log("Initializing luniverse token contract...");
